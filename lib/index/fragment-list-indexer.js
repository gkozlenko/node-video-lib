...
class FragmentListIndexer {
    ...
    static index(fragmentList, fd) {
        ...
        if (fragmentList.video) {
            buffer.writeUInt32BE(fragmentList.video.extraData.length, pos); pos += 4;
        ...
        if (fragmentList.audio) {
            buffer.writeUInt32BE(fragmentList.audio.extraData.length, pos); pos += 4;
