...
class Packetizer {
    ...
    static packetize(fragment) {
        if (!(fragment instanceof Fragment)) {
            throw new Error('Argument 1 should be instance of Fragment');
        }
        let packetizer = new Packetizer(fragment);
    ...
    constructor(fragment) {
        ...
        if (this._fragment.videoExtraData && this._fragment.videoExtraData.length > 0) {
            this._videoExtraParser = ExtraParserH264.parse(this._fragment.videoExtraData);
        }
        this._audioExtraParser = null;
        if (this._fragment.audioExtraData && this._fragment.audioExtraData.length > 0) {
            this._audioExtraParser = ExtraParserAac.parse(this._fragment.audioExtraData);
        }
    ...
    packFragment() {
        ...
        for (let sample of this._fragment.samples) {
            let packetSize = sample.buffer.readInt32BE(0) + 4;
            let packet = sample.buffer;
            let sampleTime = (90 * sample.relativeTimestamp() * this._fragment.timescale) << 0;
            ...
            if (sample instanceof AudioSample) {
                ...
                let sampleCompTime = (90 * sample.compositionOffset * this._fragment.timescale / sample.timescale) << 0;
                ...
                do {
                    ...
                    let pesPacketLength = (13 + chunkLength) & 0xff;
    ...
    _buildHeader() {
        ...
        sectionLength = 18 + (this._fragment.audioExtraData ? 5 : 0);
        mapBuf = new Buffer(17 + (this._fragment.audioExtraData ? 5 : 0));
        mapPos = 0;
        mapBuf[mapPos++] = 2;
        mapBuf[mapPos++] = ((sectionLength >> 8) & 0x0f) + 0xb0;
        mapBuf[mapPos++] = sectionLength & 0xff;
        ...
        // Audio data
        if (this._fragment.audioExtraData.length > 0) {
            mapBuf[mapPos++] = 0x0f;
