...
class Packetizer {
    ...
    _packPayload(payload, sample, pid) {
        ...
        for (let index = 0; index < numPackets; index++) {
            ...
            buffer[pos++] = pid & 0xff;
            buffer[pos++] = adaptationFields ? 0x30 : 0x10;
            if (sample instanceof AudioSample) {
                buffer[pos - 1] += this._audioCounter;
                this._audioCounter = (this._audioCounter + 1) & 0xf;
            } else if (sample instanceof VideoSample) {
                buffer[pos - 1] += this._videoCounter;
            ...
            if (adaptationFields) {
                ...
                if (0 < adaptationLength) {
                    ...
                    if (sample instanceof VideoSample && 0 === index) {
                        ...
                        buffer[pos + 6] = 0;
                        buffer[pos + 5] = ((time & 0x1) << 7) | 0x7e;
                        time >>= 1;
                        buffer[pos + 4] = time & 0xff;
                        time >>= 8;
                        buffer[pos + 3] = time & 0xff;
                        time >>= 8;
                        buffer[pos + 2] = time & 0xff;
                        time >>= 8;
    ...
    _buildHeader() {
        let buf = Buffer.allocUnsafe(2 * PACKET_SIZE);
        let pos = 0;
