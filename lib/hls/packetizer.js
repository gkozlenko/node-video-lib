...
class Packetizer {
    ...
    constructor(fragment, sampleBuffers) {
        ...
        if (this.fragment.videoExtraData) {
            this._videoExtraParser = ExtraParserH264.parse(this.fragment.videoExtraData);
        ...
        for (let i = 0, l = this.fragment.samples.length; i < l; i++) {
            ...
            if (sample instanceof AudioSample) {
                ...
                if (packet.length > packetSize + 4) {
                    packet.writeUInt32BE(1, packetSize);
                }
                if (sample.keyframe && packet.length > packetSize + 16 && packet.readUInt16BE(packetSize + 12) === 0) {
                    packet.writeUInt32BE(1, packetSize + 12);
                }
                ...
                let newPos = 0;
                newPacket.writeUInt32BE(1, newPos);
                newPos += 4;
                newPacket.writeUInt16BE(sample.keyframe ? 0x910 : 0x930, newPos);
                newPos += 2;
                ...
                newPacket.writeUInt32BE(1, newPos);
                packet.copy(newPacket, newPos + 4, 4);
                packet = newPacket;
    ...
    static _writeTime(buffer, pos, time, base) {
        buffer[pos++] = ((time >> 29) & 0x0e) | (base & 0xf0) | 0x1;
        buffer[pos++] = time >> 22;
        buffer[pos++] = (time >> 14) | 0x1;
        buffer[pos++] = time >> 7;
        buffer[pos++] = (time << 1) | 0x1;
    }
