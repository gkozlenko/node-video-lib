...
class Packetizer {
    ...
    constructor(fragment, sampleBuffers) {
        ...
        this._audioExtraParser = this.fragment.audioExtraData ? CodecParser.parse(this.fragment.audioExtraData) : null;
    ...
    _convertVideoSample(sample, isKeyframe) {
        ...
        let packet = new Buffer(7 + sample.length + bufConfig.length);
    ...
    _buildVideoConfig() {
        let codecInfo = CodecParser.parse(this.fragment.videoExtraData);
        let units = codecInfo.units();
