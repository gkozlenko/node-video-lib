...
class PacketizerImpl {
    ...
    packFragment() {
        ...
        for (let i = 0, l = this.fragment.samples.length; i < l; i++) {
            ...
            let dtsTime = TIME_OFFSET + Math.floor(PACKET_TIMESCALE * sample.timestamp / sample.timescale);

            if (sample instanceof AudioSample) {
                if (audioPackets.length === MAX_AUDIO_SAMPLES_PACK) {
                    let audioBuffer = this._packAudioPayload(audioPackets, audioPacketsSample, audioPacketsTime);
                ...
                let sampleCompTime = Math.floor(PACKET_TIMESCALE * sample.compositionOffset / sample.timescale);
                let ptsTime = dtsTime + sampleCompTime;

                let videoBuffer = this._packVideoPayload(buffer, sample, ptsTime, dtsTime);
    ...
    _packAudioPayload(buffers, sample, dtsTime) {
        let payloadLength = 14;
        for (let i = 0, l = buffers.length; i < l; i++) {
            payloadLength += 7 + buffers[i].length;
        }
        ...
        for (let i = 0, l = buffers.length; i < l; i++) {
            const packetLength = 7 + buffers[i].length;
    ...
    _packVideoPayload(buffer, sample, ptsTime, dtsTime) {
        ...
        if (this._videoCodecInfo && this._videoCodecInfo.type() === CodecUtils.CODEC_H265) {
            payloadLength++;
        }
        let data = Buffer.allocUnsafe(payloadLength);
