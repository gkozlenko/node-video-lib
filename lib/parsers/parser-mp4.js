...
function readAtom(file, offset) {
    ...
    return fs.readAsync(file, buffer, 0, buffer.length, offset).then(function() {
        ...
        if (size === 1 && type === MediaUtil.ATOM_MDAT) {
            var int64buffer = new Buffer(8);
            return fs.readAsync(file, int64buffer, 0, int64buffer.length, offset + MediaUtil.HEADER_SIZE).then(function() {
                size = MediaUtil.readUInt64BE(int64buffer, 0);
                return {
                    type: type,
                    size: size,
                    offset: offset
