...
class ExtraParserAac extends ExtraParser {

    static parse(extraData) {
        let parser = new ExtraParserAac(extraData);
        parser.parse();
        return parser;
    }
    ...
    parse() {
        ...
        if (this.rateIndex > 0 && this.rateIndex < AAC_SAMPLE_RATES.length) {
            this.sampleRate = AAC_SAMPLE_RATES[this.rateIndex];
        }
        this.channelsIndex = (flags2 & 0x7f) >> 3;
        if (this.channelsIndex > 0 && this.channelsIndex < AAC_CHANNELS.length) {
            this.channels = AAC_CHANNELS[this.channelsIndex];
        }
    }
}
