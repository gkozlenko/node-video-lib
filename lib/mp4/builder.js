...
class Builder {
    ...
    static build(movie, source, fd) {
        ...
        // Tracks and Samples
        let tracks = [movie.videoTrack(), movie.audioTrack()]
        ...
        for (let track of tracks) {
            ...
            sttsAtom.entries = samples.reduce(Builder.buildDurationsReducer, []).reduce(Builder.compressReducer, []);
            ...
            if (track instanceof VideoTrack) {
                ...
                cttsAtom.entries = samples.map((sample) => sample.compositionOffset).reduce(Builder.compressReducer, []);
            }
        ...
        for (let i = 0, l = samples.length; i < l; i++) {
            ...
            if (sample instanceof VideoSample) {
                videoOffsets.push(pos);
            } else {
                audioOffsets.push(pos);
            }
            let buffer = Buffer.allocUnsafe(sample.size);
            reader.read(buffer, sample.offset);
        ...
        for (let trackId in offsetAtoms) {
            ...
            if (track instanceof VideoTrack) {
                offsAtom.entries = videoOffsets;
            } else {
                offsAtom.entries = audioOffsets;
            }
        }
    ...
    static buildDurationsReducer(durations, sample, index, samples) {
        if (index === 0) {
            if (samples.length > 1) {
                durations.push(samples[1].timestamp - sample.timestamp);
            } else {
                durations.push(0);
            }
        } else {
            durations.push(sample.timestamp - (index > 0 ? samples[index - 1].timestamp : 0));
        }
        return durations;
    }
