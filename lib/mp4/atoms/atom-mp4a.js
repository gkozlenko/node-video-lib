...
class AtomMP4A extends Atom {
    ...
    parse(buffer) {
        ...
        while (offset < buffer.length - 8) {
            ...
            if (type === Utils.ATOM_ESDS) {
                ...
                console.log(buffer[offset], buffer[offset + 1], buffer[offset + 2], buffer[offset + 3], buffer[offset + 4]);
                offset += 5;
                ...
                if (0 < extraSize.size) {
                    this.extraData = Buffer.allocUnsafe(4 + extraSize.size);
    ...
    build(buffer, offset) {
        ...
        // extra data
        buffer.writeUInt32BE(this.extraData.length + 4, offset + 36);
        buffer.write(Utils.ATOM_ESDS, offset + 40);
        this.extraData.copy(buffer, offset + 44, 4);
    }

    bufferSize() {
        return 40 + this.extraData.length;
    }
