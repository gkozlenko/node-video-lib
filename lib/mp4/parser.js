...
class Parser {
    ...
    static parse(fd) {
        // Get moov atom
        let moovAtom = Parser._getMoovAtom(fd);
        if (!moovAtom) {
    ...
    static _getMoovAtom(fd) {
        return (function find(offset, size) {
            let atom = Parser._readAtom(fd, offset);
            if (atom) {
                if (atom.type === Utils.ATOM_MOOV) {
                    let buffer = new Buffer(atom.size - 8);
                    let read = fs.readSync(fd, buffer, 0, buffer.length, atom.offset + 8);
            ...
            return null;
        })(0, fs.fstatSync(fd).size);
    }

    static _readAtom(fd, offset) {
        let buffer = new Buffer(8);
        let read = fs.readSync(fd, buffer, 0, buffer.length, offset);
        if (read === buffer.length) {
