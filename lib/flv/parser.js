...
class Parser {
    ...
    static parse(source) {
        ...
        // Parse header
        let bufHeader = Buffer.allocUnsafe(HEADER_SIZE);
        ...
        while (pos < sourceSize) {
            ...
            if (TYPE_SCRIPT === type) {
                let bufTag = Buffer.allocUnsafe(dataSize);
                pos += reader.read(bufTag, pos);
                let data = AmfParser.parse(bufTag);
                ...
                let bufTag = Buffer.allocUnsafe(2);
                ...
                if (!setAudioData) {
                    ...
                    audioTrack.sampleRate = parseInt(5512.5 * (1 << soundRate), 10);
                ...
                if (AUDIO_FORMAT_AAC === soundFormat) {
                    ...
                    if (0 === packetType) {
                        ...
                        reader.read(extraData, pos + headerSize, 4);
                        audioTrack.extraData = extraData;
                        audioTrack.codec = CodecParser.parse(extraData).codec();
                        pos += dataSize;
                        continue;
                    }
                }
                ...
                let bufTag = Buffer.allocUnsafe(5);
                reader.read(bufTag, pos);
                ...
                if (VIDEO_FORMAT_H264 === videoFormat) {
                    ...
                    if (0 === packetType) {
                        ...
                        videoTrack.codec = CodecParser.parse(extraData).codec();
                        pos += dataSize;
                        continue;
                    }
                }
